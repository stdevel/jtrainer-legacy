...
public class JTrainer extends JFrame implements ItemListener
{
	...
	// declarations
		...
		// runtime variables
		private boolean errorFlag=false, hintUsed=false, timeOver=false;
		int thisQuestion;
	...
	// timer
		...
		private final JPanel panRadiobuttons;
		private final JPanel panTextfield;
		...
		final JMenuBar mnbMenu;
			...
			final ButtonGroup bgrSettings;
			final ButtonGroup bgrSettings2;
	...
	/* class with graphical user interface */
	{
		...
		mniExit.addActionListener(e -> {
			...
			// label panel
			JPanel panLbls = new JPanel(new GridLayout(1, 0));
	...
	/* show the next question */
	{
		...
		if(thisQuestion != -1)
		{
			...
			if(!this.thisCatalog.questions[thisQuestion].getHint().equals("NONE")) { btnHint.setEnabled(true); }
	...
	/* check answered question */
	{
		...
		if(!this.thisCatalog.questions[thisQuestion].getExplanation().equals("NONE")) { txtQuestion.append("\n\n" + this.thisCatalog.questions[thisQuestion].getExplanation()); }
